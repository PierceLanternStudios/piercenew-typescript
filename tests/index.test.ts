...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['PIERCENEW_CHANGEINCONFIG_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: PiercenewChangeinconfig) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new PiercenewChangeinconfig({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['PIERCENEW_CHANGEINCONFIG_LOG'] = 'debug';
      const client = new PiercenewChangeinconfig({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['PIERCENEW_CHANGEINCONFIG_LOG'] = 'not a log level';
      const client = new PiercenewChangeinconfig({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'PIERCENEW_CHANGEINCONFIG_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['PIERCENEW_CHANGEINCONFIG_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new PiercenewChangeinconfig({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['PIERCENEW_CHANGEINCONFIG_BASE_URL'] = 'https://example.com/from_env';
      const client = new PiercenewChangeinconfig({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['PIERCENEW_CHANGEINCONFIG_BASE_URL'] = ''; // empty
      const client = new PiercenewChangeinconfig({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3');
    });

    test('blank env variable', () => {
      process.env['PIERCENEW_CHANGEINCONFIG_BASE_URL'] = '  '; // blank
      const client = new PiercenewChangeinconfig({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['PIERCENEW_CHANGEINCONFIG_BASE_URL'] = 'http://localhost:5000/env';
      const client = new PiercenewChangeinconfig({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new PiercenewChangeinconfig({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new PiercenewChangeinconfig({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new PiercenewChangeinconfig({ apiKey: 'My API Key', fetch: testFetch });
