...
import { APIPromise } from 'piercenew/core/api-promise';

import util from 'node:util';
import Piercenew from 'piercenew';
import { APIUserAbortError } from 'piercenew';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['PIERCENEW_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Piercenew) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Piercenew({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('default logLevel is warn', async () => {
      const client = new Piercenew({ apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Piercenew({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['PIERCENEW_LOG'] = 'debug';
      const client = new Piercenew({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['PIERCENEW_LOG'] = 'not a log level';
      const client = new Piercenew({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'PIERCENEW_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['PIERCENEW_LOG'] = 'debug';
      const client = new Piercenew({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Piercenew({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['PIERCENEW_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Piercenew({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['PIERCENEW_BASE_URL'] = 'https://example.com/from_env';
      const client = new Piercenew({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['PIERCENEW_BASE_URL'] = ''; // empty
      const client = new Piercenew({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3');
    });

    test('blank env variable', () => {
      process.env['PIERCENEW_BASE_URL'] = '  '; // blank
      const client = new Piercenew({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new Piercenew({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['PIERCENEW_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Piercenew({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Piercenew({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Piercenew({ apiKey: 'My API Key' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Piercenew({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Piercenew({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Piercenew({ apiKey: 'My API Key', fetch: testFetch });
