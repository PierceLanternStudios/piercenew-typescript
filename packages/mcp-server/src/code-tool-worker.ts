...
import { Piercenew, ClientOptions } from 'piercenew';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Piercenew)`
    : `const run: (${functionSource.client}: Piercenew) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
