...
const prompt = `Runs JavaScript code to interact with the Piercenew API.
...
export function codeTool(params: { blockedMethods: SdkMethod[] | undefined }): McpTool {
  ...
  const handler = async (client: Piercenew, args: any): Promise<ToolCallResult> => {
    ...
    const res = await fetch(codeModeEndpoint, {
      ...
      headers: {
        ...
        client_envs: JSON.stringify({
          ...
          PIERCENEW_BASE_URL: readEnv('PIERCENEW_BASE_URL') ?? client.baseURL ?? undefined,
        }),
      },
      body: JSON.stringify({
        project_name: 'piercenew',
        code,
        intent,
        client_opts: {},
